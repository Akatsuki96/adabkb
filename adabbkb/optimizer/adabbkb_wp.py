...
class AdaBBKBWP(AbsOptimizer):
    ...
    def _extend_search_space(self, leaf_set):
        ...
        for node in leaf_set:
            if tuple(node.x) not in self.node2idx:
                ...
                self.num_nodes += 1
                self.ucbs_vh = np.append(self.ucbs_vh, 0)
    ...
    def initialize(self, target_fun, search_space, N : int = 2, budget : int = 1000, h_max: int = 10):
        ...
        while ne < budget:
            ...
            if self.beta * root_std <= Vh_root:
                ...
                self.ucbs_vh = np.zeros(len(self.leaf_set))
                ...
                self._update_mean_variances(idx_to_update=to_upd)
                self._compute_index()
                self._prune_leaf_set()
                return self.leaf_set
        raise Exception("Initialization not completed! You should increase budget!")



    def _prune_leaf_set(self):
        to_discard = self.ucbs_vh < self.best_lcb[1]
        num_discarted = len(self.ucbs_vh[to_discard])
        if self.verbose:
            print("[--] best lcb: {}\tn. discarted cells: {}\tLeaf set size: {}".format(self.best_lcb[1], num_discarted, len(self.leaf_set)))
        if num_discarted > 0:
            self.num_pruning+= num_discarted
            self.I = np.delete(self.I, to_discard, 0)
            self.ucbs_vh = np.delete(self.ucbs_vh, to_discard, 0)
            self.leaf_set = np.delete(self.leaf_set, to_discard, 0)
    ...
    def _compute_index(self, lfset_indices = None):
        ...
        for i in lfset_indices:
            ...
            Vh = self._compute_V(node.level)
            ucb = self.means[self._get_node_idx(node)] +self.beta * np.sqrt(self.variances[self._get_node_idx(node)])
            self.ucbs_vh[i] =  ucb + Vh
            self.I[i] = np.min([
                            ucb,
                            self.means[self._get_node_idx(node.father)] +
                            self.beta * np.sqrt(self.variances[self._get_node_idx(node.father)]) + \
                            self._compute_V(node.father.level) 
                        ]) + Vh

    def _can_be_expanded(self, node_idx, h, Vh):
        return np.sqrt(self.variances[node_idx]) * self.beta <= Vh and (self.h_max is None or h < self.h_max)

    def _evaluate_index(self, leaf_idx):
        node = self.leaf_set[leaf_idx]
        node_idx = self._get_node_idx(node)
        father_idx = self._get_node_idx(node.father)
        return np.min([
                self.means[node_idx] + np.sqrt(self.variances[node_idx]),
                self.means[father_idx] + np.sqrt(self.variances[father_idx]) + 
                self._compute_V(node.father.level)         
        ...
        while True:
            ...
            if self._can_be_expanded(node_idx, selected_node.level, Vh):
                ...
                if len(self.leaf_set) == 0:
                    return np.array(self.best_lcb[0]), None
                tmp_I = self.I.copy()
                tmp_variances = self.variances.copy()
            else:
...
class AdaBBKBWP_2(AbsOptimizer):
    def __init__(self, dot_fun: Kernel, ratio_threshold : float = 2.0, options: OptimizerOptions = None):
        self.ratio_threshold = ratio_threshold
        self.global_ratio_bound = 1.0
        super().__init__(dot_fun, options)
    ...
    def _compute_index(self, lfset_indices = None):
        ...
        for i in lfset_indices:
            node = self.leaf_set[i]
            Vh = self._compute_V(node.level)
            ucb = self.means[self.node2idx[tuple(node.x)]] + self.beta * np.sqrt(self.variances[self.node2idx[tuple(node.x)]])
            self.ucbs_vh[i] =  ucb + Vh
            self.I[i] = np.min([
                            ucb,
